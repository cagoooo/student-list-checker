...
- Firebase Functions 後端校對第一版：登入後 `.xlsx` / `.csv` / `.docx` / 文字型 PDF 可先交由後端解析、讀正式學生資料庫並回傳問題清單
- 後端校對會保存摘要紀錄編號，方便老師回報與資訊組長追蹤
- 掃描 PDF 會建立 OCR 背景工作、暫存到短期 Storage path，並由 worker 非同步處理；影像 OCR 引擎仍保留為下一步接入點
