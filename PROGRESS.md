...
| PDF 名單解析 | 已完成 | 抽取 PDF 純文字並偵測「班級 座號 姓名」列 | 透過 `src/lib/importer/pdf.ts`；掃描 / 影像 PDF 之 OCR 留待後續 |
| Word 名單解析 | 已完成 | 讀取 `.docx` 表格與段落抽取名單列 | 透過 `src/lib/importer/word.ts` |
...
| 項目 | 狀態 | 說明 |
|---|---|---|
| PDF / Word 解析 | 部分完成 | 已支援文字型 PDF 與 `.docx` 表格 / 段落抽取；掃描影像 PDF 的 OCR、複雜多欄版面仍待加強 |
...
| P2 | 強化 PDF / Word 辨識（OCR、多欄、原始位置標註） | 文字型已支援，擴大涵蓋掃描與複雜版面 |
