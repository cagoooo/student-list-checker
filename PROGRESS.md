...
| 問題清單排序 | ✅ 已完成 | 需要老師確認的項目會先列錯誤，再列待確認；同狀態內依班級、來源、列號、信心與姓名排序 | `src/App.tsx` |
...
| 後端問題排序 | ✅ 已完成 | Firebase Functions 回傳的 issues 與前端一致：錯誤優先，再依班級、來源、列號、信心與姓名排序 | `functions/src/index.ts`，尚待部署 |
...
| 問題排序策略 | ✅ 已完成第一版 | 問題清單依錯誤嚴重度、班級、來源、列號、信心與姓名排序，讓老師可按班級順序回原始檔檢查 |
...
| 問題清單排序 | 原本待確認項目看起來像亂序，尤其 `中階 201` 會因來源排序被排到 `402`、`505` 後面 | 排序改為錯誤優先；同狀態先按班級，再按來源、列號、信心、姓名 | 待確認清單可依班級順序檢查，例如 `104 → 105 → 201 → 402 → 505` |
