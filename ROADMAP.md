...
已完成第一版：問題清單會先列 `錯誤`，再列 `待確認`；同狀態內改成先按班級排序，再按來源、列號、信心與姓名排序。下一步可以把排序升級成老師更直覺的「檢查動線」。

建議功能：

1. 顯示目前排序規則，例如「排序：狀態 → 班級 → 來源 → 列號」。
2. 提供排序切換：依班級、依來源、依信心、依原始列號。
3. 依班級自動分組，顯示 `104 班 2 筆待確認`、`201 班 1 筆待確認`。
4. 每個班級分組可折疊，老師檢查完一班就收起來。
5. 同一班內優先顯示錯誤，再顯示待確認。
6. 保留「原始檔順序」切換，方便老師對照 Excel 第幾列。

建議 UI：

```txt
排序方式：[班級順序] [來源順序] [信心高低] [原始列號]
...
價值：老師通常是回 Excel 或紙本一班一班檢查，班級分組比單純列表更貼近實際修正流程。

---

### P1｜問題類型篩選與快速定位

**[難度] 小　[需要] 問題分類 metadata**

目前已能區分 `錯誤` 與 `待確認`，後續可增加更細的篩選條件，讓問題多時仍然好查。
...
2. 只看待確認。
3. 只看某一班。
4. 只看某一來源，例如只看 `初階` 或 `中階`。
5. 只看低信心項目，例如信心小於 80%。
6. 只看「查無姓名」或「姓名近似」。

價值：當名單問題從 7 筆變成 70 筆時，老師仍能快速聚焦處理。

---

### P2｜問題清單檢查狀態與本機暫存

**[難度] 中　[需要] localStorage 或 Firestore validations 延伸欄位**

老師實際修正時，可能會邊看系統、邊改 Excel。建議讓每一筆問題可以標記處理狀態。
...
|---|---|
| 未處理 | 預設狀態 |
| 已回原檔確認 | 老師已看過原始 Excel |
| 已修正 | 老師已修改原始檔 |
| 略過 | 判定不是問題，例如同名、特殊字、資料庫未更新 |

可先存在瀏覽器 localStorage；若 Firebase 正式部署後，再寫進 `validations/{id}` 的個人紀錄。

價值：長名單校對不一定一次完成，檢查狀態可避免重複看同一筆。

---

### P2｜依目前排序匯出 / 複製問題清單

**[難度] 小　[需要] 現有 displayIssues 即可**

問題排序穩定後，匯出與複製也應沿用同一順序。

建議：

1. 「複製問題清單」照畫面排序輸出。
2. 「下載問題清單 Excel」照畫面排序輸出。
3. 匯出欄位加入 `排序順位`，方便行政人員追蹤。
4. 分組輸出：每班一段，或每個工作表一個分頁。

價值：老師可以直接把排序好的問題清單貼給導師或行政同仁，不必重新整理。
...
| **P1** | 問題清單排序、分組與檢查動線 | 小到中 | 讓老師能依班級順序一班一班回原檔檢查 |
| **P1** | 問題類型篩選與快速定位 | 小 | 問題多時可只看錯誤、某班、某來源或低信心項目 |
...
| **P2** | 問題清單檢查狀態與本機暫存 | 中 | 長名單可標記已確認、已修正或略過 |
| **P2** | 依目前排序匯出 / 複製問題清單 | 小 | 匯出與貼給同事時保留畫面排序與班級分組 |
